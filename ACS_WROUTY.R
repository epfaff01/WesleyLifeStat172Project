...
source("Final Project Materials/code/predict_wrouty.R") #Lasso/ridge
source("Final Project Materials/code/ACSCleaning.R") #clean ACS

# Prepare ACS data for predictions
acs_wrouty <- as.matrix(subset(acs_data, select = -c(serialno, PUMA, weight,FAMINC_numeric)))

# Generating predictions using Ridge regression (received a better score than Lasso in our CPS predictions)
acs_ridge_pred = predict(final_ridge, acs_wrouty, type = "response")[,1] # Using ridge, it's a hair better
...
# Calculate the predicted number of seniors worried food will run out
results$FSWROUTY_senior <- results$weighted_mean * results$senior_population

# Merge data to visualize the senior predictions
map_data <- puma_shapefile_simplified %>%
  left_join(results, by = c("GEOID20" = "PUMA"))

# Plot the predicted number of seniors worried food will run out
ggplot(data = map_data) +
  geom_sf(aes(fill = FSWROUTY_senior), color = "black") +
  scale_fill_gradient(low = "white", high = "black", name = "Seniors") +  
  theme_minimal() +
  labs(
    title = "Predicted Seniors Worried Food Will Run Out",
    caption = "Data source: ACS dataset and Ridge Regression Model"
  ) +
  theme(legend.position = "right")