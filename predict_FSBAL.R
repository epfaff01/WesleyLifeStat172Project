...
source("Final Project Materials/code/clean_cps.R")

# Subset data: remove rows with missing FSBAL and irrelevant columns
cps_bal <- subset(cps_data, !is.na(cps_data$FSBAL),
                  select = -c(FSTOTXPNC_perpers, FSSTATUS, FSSTATUSMD, FSFOODS,
                              FSWROUTY, FSRAWSCRA, FSTOTXPNC, CPSID, COUNTY, FAMINC_numeric))
...
train.idx <- sample(x = 1:nrow(cps_bal), size = 0.7*nrow(cps_bal))
train.df <- cps_bal[train.idx,]
test.df <- cps_bal[-train.idx,]
...
x.train <- model.matrix(FSBAL ~ ., data = train.df)[, -1]  # Training matrix (exclude intercept)
x.test <- model.matrix(FSBAL ~ ., data = test.df)[, -1]  # Testing matrix (exclude intercept)
...
test.df.preds <- test.df %>% 
  mutate(
    lasso_pred = predict(final_lasso, x.test, type = "response")[,1],  # Lasso predictions
    ridge_pred = predict(final_ridge, x.test, type = "response")[,1]  # Ridge predictions
  )
